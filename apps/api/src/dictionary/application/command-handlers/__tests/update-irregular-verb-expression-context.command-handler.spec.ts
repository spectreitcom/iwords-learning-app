...
import { UpdateIrregularVerbExpressionContextCommandHandler } from '../update-irregular-verb-expression-context.command-handler';
...
describe('UpdateIrregularVerbExpressionContextCommandHandler', () => {
  ...
  let handler: UpdateIrregularVerbExpressionContextCommandHandler;
  ...
  beforeEach(() => {
    ...
    handler = new UpdateIrregularVerbExpressionContextCommandHandler(
  ...
  it('should update an irregular verb expression context', async () => {
    ...
    const expressionContext = new ExpressionContext(
      ...
      'old translation',
      false,
      ExpressionType.fromString(IRREGULAR_VERB),
      null,
      false,
      null,
      null,
    ...
    const newForms: [string, string, string] = ['new1', 'new2', 'new3'];
    ...
    expect(updatedContext?.getForms()?.value).toEqual(
      newForms.map((f) => f.toLowerCase()),
    );
