...
import { CreateIrregularVerbExpressionContextCommandHandler } from '../create-irregular-verb-expression-context.command-handler';
...
describe('CreateIrregularVerbExpressionContextCommandHandler', () => {
  ...
  let handler: CreateIrregularVerbExpressionContextCommandHandler;
  ...
  beforeEach(() => {
    ...
    handler = new CreateIrregularVerbExpressionContextCommandHandler(
      eventPublisher,
  ...
  it('should create an irregular verb expression context', async () => {
    ...
    const forms: [string, string, string] = ['form1', 'form2', 'form3'];
    ...
    expect(eventPublisher.lastMerged.getForms()?.value).toEqual(
      forms.map((f) => f.toLowerCase()),
    );
    expect(eventPublisher.lastMerged.getIsIrregular()).toBe(true);
